...
import { UNAME, PASSWD, KEY_GENERATOR_URL, KNOWN_KEY_URL, NO_MORE_LEAKS_HEADER, POSITIVE_MATCH_URL, } from "./constants.js";
...
export async function responseProvider(request) {
    ...
    if (body && isValidBody(body)) {
        ...
        // if we have received some response, let's validate the key against our db.
        if (key) {
            // to get subWorker logs, use Pragma:akamai-x-ew-debug-rp,akamai-x-ew-subworkers,akamai-x-ew-debug-subs in request header
    ...
    if (originResponse) {
        ...
        return Promise.reject(`failed origin sub-request: ${originResponse.status}`);
    }
}
...
async function getKeyFromKeyGenerator(requestBody) {
    try {
        ...
        if (result.ok) {
            ...
            logger.error(`Request to ${KEY_GENERATOR_URL} failed with status: ${result.status}`);
        }
    }
    catch (error) {
...
async function keyExists(key, auth) {
    /*
    ...
    try {
        ...
        if (result.ok) {
            ...
            if (response.hasOwnProperty("id") &&
                ...
                logger.info(`Call to ${KNOWN_KEY_URL} found id ${response["id"].substring(0, 5)}.....`);
                return response["id"];
            }
            else {
                logger.error("id field not in json response so key not found or wrong format");
            }
        }
    }
    catch (error) {
...
async function originRequest(request, body, id, informHeader = NO_MORE_LEAKS_HEADER) {
    /*
...
function registerPositiveMatch(reqBody, auth) {
    /*
